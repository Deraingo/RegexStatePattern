...
class RegexStatePatternTests {
    ...
    fun `test invalid floating point values`() {
        val invalidInputs = listOf("", "123.123.", "123.02a", "123.a", "012.a")
    ...
    fun `test valid binary numbers`() {
        ...
        for (input in validInputs) {
            val detector = BinaryNumberDetector()
    ...
    fun `test valid email addresses`() {
        val validInputs = listOf("a@b.com", "joseph.ditton@usu.edu", "a02202949@aggies.usu.edu")
    ...
    fun `test invalid email addresses`() {
        val invalidInputs = listOf("@b.c", "a@b@c.com", "f%od@@..com", "joseph ditton@usu.edu")
